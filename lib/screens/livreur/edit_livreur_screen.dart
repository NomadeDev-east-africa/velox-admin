...
/// Modification d'un livreur existant (profil Firestore : identité + moto +
/// statut + photo). Ne touche **pas** au compte Firebase Auth ni au mot de
/// passe — pour cela, il faudrait supprimer puis recréer le livreur.
class EditLivreurScreen extends StatefulWidget {
  final String livreurId;
  final Map<String, dynamic> data;

  const EditLivreurScreen({
    super.key,
  ...
  @override
  State<EditLivreurScreen> createState() => _EditLivreurScreenState();
}
...
class _EditLivreurScreenState extends State<EditLivreurScreen> {
  ...
  // Photo : URL existante conservée si aucune nouvelle image sélectionnée.
  ...
  Future<String?> _uploadImage() async {
    ...
    try {
      ...
      final ref = FirebaseStorage.instance
          .ref()
          .child('livreurs')
          .child('livreur_${widget.livreurId}_$ts.jpg');
      await ref.putData(
          _imageBytes!, SettableMetadata(contentType: 'image/jpeg'));
      return await ref.getDownloadURL();
    } catch (_) {
      // En cas d'échec d'upload, on conserve l'ancienne photo.
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await _firestore.collection('livreurs').doc(widget.livreurId).update({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Modifier le Livreur'), elevation: 0),
          ...
          child: Container(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  _sectionHeader(Icons.photo_camera, 'Photo du livreur'),
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(largePadding),
                      child: Center(
                  ...
                  const SizedBox(height: largePadding),
                  _sectionHeader(Icons.motorcycle, 'Informations Moto'),
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(largePadding),
                      child: Column(
